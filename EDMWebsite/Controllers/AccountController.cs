...
namespace EDMWebsite.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser() { UserName = model.Name, Email = model.Email, NickName = model.Name };
                ...
                if (result.Succeeded)
                {
                    ...
                    await UserManager.SendEmailAsync(user.Id, "激活账号", "请点击 <a href=\"" + callbackUrl + "\">这里</a>激活你的账号。");
                    return RedirectToAction("ConfirmEmail", "Home", new { userId = user.Id, code = code });
                }
                else
                {
                    AddErrors(result);
        ...
        public async Task<ActionResult> ForgotPassword(ForgotPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                {
                    ModelState.AddModelError("", "这个用户不存在或者没有激活！The user either does not exist or is not confirmed.");
                    return View("Login");
                }
