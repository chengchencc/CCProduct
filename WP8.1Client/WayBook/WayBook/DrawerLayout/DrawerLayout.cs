...
namespace DrawerLayout
{
    public class DrawerLayout : Grid
    {
        ...
        public void InitializeDrawerLayout()
        {
            ...
            var animatedTranslateTransform = new TranslateTransform { X = -_listLeftFragment.Width, Y = 0 };
        ...
        private void shadowFragment_Tapped(object sender, TappedRoutedEventArgs e)
        {
            if (IsLeftDrawerOpen)
            {
                ...
                shadow.Completed += shadowLeft_Completed;
                shadow.Begin();
            }
            else
            {
                ...
                Storyboard.SetTarget(doubleAnimation, _listRightFragment);
                Storyboard.SetTargetProperty(doubleAnimation, TranslatePath.Path);
                shadow.Children.Add(doubleAnimation);
                ...
                shadow.Completed += shadowRight_Completed;
                shadow.Begin();
            }
        }
        private void shadowLeft_Completed(object sender, object e)
        {
            _shadowFragment.IsHitTestVisible = false;
        ...
        private void shadowRight_Completed(object sender, object e)
        {
            _shadowFragment.IsHitTestVisible = false;
            _shadowFragment.Visibility = Visibility.Collapsed;
        ...
        private void mainLeftFragment_ManipulationStarted(object sender, ManipulationStartedRoutedEventArgs e)
        {
            ...
            if (!(e.Position.X <= 40) && !(e.Position.X >= Window.Current.Bounds.Width - 40)) return;
        ...
        {
            if (Math.Abs(e.Cumulative.Translation.X) < 0) return;
            if (e.Cumulative.Translation.X >= _listLeftFragment.Width)
            {
