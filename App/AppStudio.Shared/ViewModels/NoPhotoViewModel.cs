...
{
    public class NoPhotoViewModel : ViewModelBase<NoPhotoSchema>
    {
        private RelayCommandEx<NoPhotoSchema> itemClickCommand;
        public RelayCommandEx<NoPhotoSchema> ItemClickCommand
        {
            get
            {
                if (itemClickCommand == null)
                {
                    itemClickCommand = new RelayCommandEx<NoPhotoSchema>(
                        (item) =>
                        {
        ...
        override protected DataSourceBase<NoPhotoSchema> CreateDataSource()
        {
            return new NoPhotoDataSource((Guid)ApplicationData.Current.LocalSettings.Values[LocalSettingNames.StoreIdSetting], ApplicationData.Current.LocalSettings.Values[LocalSettingNames.DeviceTypeSetting] as string); // CollectionDataSource
        }
