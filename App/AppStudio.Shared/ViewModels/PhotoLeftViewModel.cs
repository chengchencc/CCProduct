...
{
    public class PhotoLeftViewModel : ViewModelBase<PhotoLeftSchema>
    {
        private RelayCommandEx<PhotoLeftSchema> itemClickCommand;
        public RelayCommandEx<PhotoLeftSchema> ItemClickCommand
        {
            get
            {
                if (itemClickCommand == null)
                {
                    itemClickCommand = new RelayCommandEx<PhotoLeftSchema>(
                        (item) =>
                        {
        ...
        override protected DataSourceBase<PhotoLeftSchema> CreateDataSource()
        {
            return new PhotoLeftDataSource((Guid)ApplicationData.Current.LocalSettings.Values[LocalSettingNames.StoreIdSetting], ApplicationData.Current.LocalSettings.Values[LocalSettingNames.DeviceTypeSetting] as string); // CollectionDataSource
        }
